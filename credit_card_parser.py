...
class CreditCardParser:
    """Main parser class with bank-specific adapters and OCR support"""
    ...
    def parse_statement(self, pdf_path: str, password: str = None) -> CreditCardData:
        """Main parsing function with encrypted PDF and OCR support"""
        # First, check if PDF is encrypted and handle it
        decrypted_pdf_path = self._check_and_decrypt_pdf(pdf_path, password)
        ...
        try:
            with pdfplumber.open(decrypted_pdf_path) as pdf:
                ...
                if not text.strip():
                    raise ValueError("No text found in PDF. This might be a scanned document. Please ensure OCR is installed.")
    ...
    def _check_and_decrypt_pdf(self, pdf_path: str, password: str = None) -> str:
        """
        ...
        try:
            ...
            with open(pdf_path, 'rb') as file:
                ...
                if reader.is_encrypted:
                    # Try to decrypt with provided password or empty string
                    decrypt_password = password if password else ""
                    
                    if reader.decrypt(decrypt_password):
            ...
            try:
                ...
                if password:
                    with pdfplumber.open(pdf_path, password=password) as pdf:
                        # If we get here, pdfplumber could open it with the password
                        return pdf_path
                else:
                ...
                if "encrypted" in str(pdfplumber_error).lower() or "password" in str(pdfplumber_error).lower():
                    raise ValueError("PDF is encrypted but no password or wrong password provided")
                else:
                    raise ValueError(f"Error reading PDF: {pdfplumber_error}")

    def _extract_text(self, pdf) -> str:
        """Extract text from PDF with multiple strategies including OCR"""
        text = ""
        ...
        # If text is too short (likely scanned), try OCR
        if len(text.strip()) < 100:
            if self.enable_ocr:
                print("⚠️  Limited text found, attempting OCR...")
                self._ocr_used = True
                text = self._extract_text_with_ocr(pdf)
            else:
                raise ValueError("No text found in PDF. This appears to be a scanned document. Please install Tesseract OCR to process scanned PDFs.")
    ...
    def _extract_text_with_ocr(self, pdf) -> str:
        ...
        for page_num, page in enumerate(pdf.pages, 1):
            try:
                ...
                print(f"✅ OCR processed page {page_num}/{total_pages}")
                
            except Exception as e:
                print(f"⚠️  OCR failed for page {page_num}/{total_pages}: {str(e)}")
                # Continue with other pages even if one fails
                continue
        
        if len(text.strip()) < 100:
            raise ValueError("OCR failed to extract sufficient text. The scan quality may be too low or the document might be corrupted. Try a higher resolution scan (300+ DPI) or download a digital copy from your bank.")
...
def main():
    ...
    print("✅ Now supports encrypted PDFs!")
    if OCR_AVAILABLE:
        print("✅ OCR enabled - Can process scanned PDFs")
    else:
    ...
    try:
        ...
        with open(pdf_path, 'rb') as file:
            ...
            if reader.is_encrypted:
                ...
                data = parser.parse_statement(pdf_path, password=password)
            else:
